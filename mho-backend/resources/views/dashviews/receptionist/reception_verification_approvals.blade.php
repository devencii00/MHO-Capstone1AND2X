...
<div id="recVerifDocPanelOverlay" class="hidden fixed inset-0 z-[80] bg-slate-900/40 items-center justify-center p-4">
    <div class="w-full max-w-4xl h-[90vh] max-h-none rounded-2xl bg-white border border-slate-200 shadow-[0_12px_30px_rgba(15,23,42,0.24)] flex overflow-hidden">
        ...
        <div class="w-1/2 border-r border-slate-200 flex flex-col min-h-0">
            <div class="px-4 py-3 border-b border-slate-100 shrink-0 flex items-center justify-between">
                <div>
                    ...
                    <div id="recVerifDocPanelSubtitle" class="text-[0.72rem] text-slate-500">Loading...</div>
                </div>
                <button type="button" id="recVerifDocPanelClose" class="text-slate-400 hover:text-slate-600">
                    <x-lucide-x class="w-[20px] h-[20px]" />
                </button>
            </div>
            <div id="recVerifHistoryBody" class="flex-1 overflow-y-auto p-3 space-y-2">
        ...
        <div class="w-1/2 flex flex-col min-h-0 bg-slate-50/50">
            ...
            <div class="shrink-0 px-4 py-2 border-b border-slate-200 bg-white flex items-center gap-2">
                <button type="button" id="recVerifApproveBtn" class="flex-1 px-3 py-2 rounded-xl border border-emerald-200 bg-emerald-50 text-[0.78rem] font-semibold text-emerald-700 text-center hover:bg-emerald-100 disabled:opacity-50 disabled:cursor-not-allowed">Approve</button>
                <button type="button" id="recVerifRejectBtn" class="flex-1 px-3 py-2 rounded-xl border border-rose-200 bg-rose-50 text-[0.78rem] font-semibold text-rose-700 text-center hover:bg-rose-100 disabled:opacity-50 disabled:cursor-not-allowed">Reject</button>
            </div>
            <div class="flex-1 overflow-y-auto p-4">
                <div id="recVerifPatientCard" class="rounded-xl border border-slate-200 bg-white p-3 mb-4">
                ...
                <div id="recVerifDocMainWrap" class="rounded-xl border border-slate-200 bg-white overflow-hidden h-[16rem] flex items-center justify-center text-[0.78rem] text-slate-500 transition cursor-default">
...
<script>
    ;(function () {
        ...
        var panelApproveBtn = document.getElementById('recVerifApproveBtn')
        var panelRejectBtn = document.getElementById('recVerifRejectBtn')
                ...
                html += '<tr class="border-b border-slate-50 last:border-0">' +
                    ...
                    '<td class="py-2 pr-4 text-[0.78rem]">' +
                        '<div class="flex items-center gap-2 flex-nowrap min-w-[19rem]">' +
                            ...
                            '' +  // approve/reject buttons moved to panel
                        '</div>' +
                    '</td>' +
                '</tr>'
            })
        ...
        function renderHistory(rows) {
            ...
            historyBody.innerHTML = items.map(function (entry) {
                ...
                return '<div class="rounded-xl border border-slate-200 bg-white p-3 hover:border-green-200 transition-colors cursor-pointer rec-verif-history-card" data-id="' + escapeHtml(id) + '">' +
            ...
            historyBody.querySelectorAll('.rec-verif-history-card').forEach(function (card) {
        ...
        function bindRowActions() {
            ...
            // approve/reject button handlers moved to panel
        }
        ...
        if (!window.__receptionVerifMouseMoveBound) {
            window.__receptionVerifMouseMoveBound = true
        ...
        if (!window.__receptionVerifKeydownBound) {
            window.__receptionVerifKeydownBound = true
