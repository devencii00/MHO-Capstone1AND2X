        ...
        if (typeof window.Echo !== 'undefined' && window.Echo) {
            ...
            window.Echo.private('appointments.all')
                .listen('.appointment.updated', function (e) {
                    ...
                    // Refresh main Select Schedule modal (calendar counters + time slots) if open
                    // NOTE: avoid loadDoctorSchedulesAndAvailability — it calls clearAvailability() which closes the modal
                    if (scheduleOverlay && !scheduleOverlay.classList.contains('hidden')) {
                        var sDocId = doctorSelect && doctorSelect.value ? String(doctorSelect.value) : ''
                        if (sDocId) {
                            if (typeof loadMonthAppointments === 'function') loadMonthAppointments(sDocId)
                            if (dateInput && dateInput.value && typeof loadDoctorAppointments === 'function') {
                                loadDoctorAppointments(sDocId, dateInput.value)
                            }
                        }
                    }
