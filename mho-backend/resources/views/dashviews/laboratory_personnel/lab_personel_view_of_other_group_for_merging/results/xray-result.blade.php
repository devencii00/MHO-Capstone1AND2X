...
@section('content')
<div class="container-fluid xray-result-container">

    <!-- Top Bar -->
    <div class="mb-3 xray-topbar no-print">
        <a href="{{ url()->previous() }}" class="back-link">
            <i class="fas fa-arrow-left"></i>
        </a>
        <div class="xray-topbar-icon">
            <i class="fas fa-lungs"></i>
        </div>
        <span class="xray-topbar-title">X-RAY RESULT</span>

        <div class="xray-topbar-actions">
    ...
    <div class="xray-result-card" id="printArea">
            ...
            <div class="patient-info-bottom">
                <div class="patient-info-grid">
                    ...
                    <div class="info-item">
                        ...
                        <span class="info-value">{{ $record->department ?? 'Radiology' }}</span>
                    </div>
                    <div class="info-item">
        ...
        <div class="xray-main-flex">

            <!-- X-ray Image (shrinks when the diagnosis drawer opens) -->
            <div class="xray-panel-wrapper">
                <div class="xray-panel">
                    <div class="panel-header">
                        <span class="panel-title">X-RAY IMAGE</span>
                        <div class="panel-header-actions no-print">
                            <label for="xrayImageInput" class="btn-upload">
                    ...
                    <!-- Hidden file input -->
                    <input type="file" id="xrayImageInput" accept="image/*" style="display: none;" onchange="previewXrayImage(event)">

                    <div class="xray-image-wrapper" id="xrayImageWrapper" onclick="openXrayLightbox()">
                        @if($record->xray_image ?? false)
                            <img src="{{ asset('storage/' . $record->xray_image) }}" alt="X-Ray Image" id="xrayImagePreview" class="xray-image">
                        @else
                            <div class="xray-image-placeholder" id="xrayImagePreviewPlaceholder">
                                <i class="fas fa-x-ray"></i>
                                <span>No image uploaded</span>
                            </div>
                            <img src="" alt="X-Ray Image" id="xrayImagePreview" class="xray-image d-none" style="display:none;">
                        @endif
                    </div>

                    <div class="xray-image-footer no-print">
            ...
            <div class="diagnosis-drawer no-print" id="diagnosisDrawer">
                <div class="diagnosis-drawer-inner">
                    ...
                    <!-- IMPRESSION CARD -->
                    <div class="diag-card diag-card-impression">
                        <div class="diag-card-header">
                            <div class="diag-card-icon">
...
<!-- Fullscreen Lightbox -->
<div class="xray-lightbox" id="xrayLightbox" onclick="closeXrayLightbox(event)">
    <button class="xray-lightbox-close" onclick="closeXrayLightbox(event)" title="Close">
        <i class="fas fa-times"></i>
    </button>
    <img id="xrayLightboxImage" class="xray-lightbox-image" alt="X-Ray Full View" onclick="event.stopPropagation()">
</div>
...
<style>
    :root {
        ...
        --xray-border: #E2E6EC;
        --xray-blue: #2F4CDD;
        --xray-blue-dark: #1F3AAE;
        --xray-blue-soft: #EEF1FD;
        --xray-text-dark: #1F2937;
        --xray-text-muted: #6B7280;
        --xray-red: #E2433B;
        --xray-green: #22C55E;
        --xray-shadow: 0 1px 3px rgba(0,0,0,0.04), 0 1px 2px rgba(0,0,0,0.03);
    }

    .xray-result-container {
        padding: 24px 0;
    ...
    .xray-topbar {
        ...
        align-items: center;
        gap: 14px;
        background-color: var(--xray-card-bg);
        border: 1px solid var(--xray-border);
        border-radius: 10px;
        padding: 16px 20px;
        box-shadow: var(--xray-shadow);
    }
    ...
    .xray-topbar-icon {
        width: 32px;
        height: 32px;
        background-color: var(--xray-blue);
        color: #fff;
        border-radius: 8px;
        display: inline-flex;
        align-items: center;
    ...
    .btn-topbar-save {
        ...
        padding: 0 18px;
        border-radius: 8px;
        background-color: var(--xray-green);
        border: 1px solid var(--xray-green);
        color: #fff;
        font-size: 13.5px;
        font-weight: 700;
        cursor: pointer;
    ...
    .btn-upload {
        ...
        border: 1px solid var(--xray-blue);
        border-radius: 6px;
        padding: 7px 14px;
        cursor: pointer;
        transition: all 0.2s;
    ...
    .btn-upload:hover {
        background-color: var(--xray-blue-soft);
    }
    ...
    .btn-diagnosis:hover {
        background-color: var(--xray-blue-dark);
    }

    .btn-diagnosis.active {
        background-color: var(--xray-blue-dark);
        box-shadow: 0 0 0 3px rgba(47, 76, 221, 0.2);
    }
    ...
    /* Diagnosis cards, styled like rounded bordered cards */
    .diag-card {
        background-color: #fff;
        border: 1px solid var(--xray-border);
        border-radius: 14px;
    ...
    .findings-list-print {
        ...
        flex-direction: column; gap: 10px; font-size: 13.5px; color: var(--xray-text-dark);
    }

    .impression-text-print {
        display: none; margin: 0; font-size: 13.5px; color: var(--xray-text-dark); line-height: 1.6;
    }
    ...
    .xray-lightbox.active { display: flex; }
    ...
    .xray-lightbox-close:hover { background-color: rgba(255, 255, 255, 0.2); }
    ...
    @media (max-width: 992px) {
        ...
        .xray-main-flex { flex-direction: column; }
        ...
        .xray-image-wrapper { min-height: 260px; }
        .xray-topbar { flex-wrap: wrap; row-gap: 10px; }
    }
    ...
    @media (max-width: 576px) {
        ...
        .xray-image-wrapper { min-height: 220px; }
...
<script>
    function previewXrayImage(event) {
        const file = event.target.files[0];
        if (!file) return;

        const preview = document.getElementById('xrayImagePreview');
        const placeholder = document.getElementById('xrayImagePreviewPlaceholder');
        ...
        fetch("{{ route('staff1.results.xray.upload', $record->id ?? '') }}", {
            method: 'POST',
            body: formData,
    ...
    function confirmRemoveImage() {
        ...
        fetch("{{ route('staff1.results.xray.remove', $record->id ?? '') }}", {
            method: 'POST',
            headers: {
        ...
        .then(() => {
            ...
            document.getElementById('xrayImageInput').value = '';
    ...
    function saveAndSubmit(event) {
        ...
        const xrayImage = document.getElementById('xrayImagePreview');
        ...
        if (!xrayImage.src || xrayImage.classList.contains('d-none') || xrayImage.style.display === 'none') {
            errors.push(' X-ray image is required');
        }

        // Check if findings is not empty
        if (!findings) {
            errors.push(' Findings cannot be empty');
        }

        // Check if impression is not empty
        if (!impression) {
        ...
        /*
         * FIX: previously this sent findings as a bare array of lines
         * (JSON.stringify(findingsArray)) and NEVER included `impression`
         * anywhere in the payload — it was only read above for validation,
         * then discarded. The server (apiSave) only fills in `impression`
         * when it receives a {findings_text, impression} object; a bare
         * array always gets impression forced to ''. That mismatch is why
         * "No impression recorded." showed up even after typing one.
         *
         * Now we send findings as a proper object with BOTH keys, matching
         * exactly what apiSave() expects for xray/ultrasound records.
         */
        const data = {
            service_type: 'xray',
        ...
        .then(data => {
            if (data.success) {
                ...
                setTimeout(() => {
                    // Redirect to xray-records dashboard
                    window.location.href = "{{ route('staff1.xray') }}";
                }, 1500);
            } else {
    ...
    function openXrayLightbox() {
        const preview = document.getElementById('xrayImagePreview');
        if (!preview.src || preview.classList.contains('d-none')) return;

        const lightbox = document.getElementById('xrayLightbox');
        if (lightbox.parentElement !== document.body) {
            document.body.appendChild(lightbox);
        }

        const lightboxImg = document.getElementById('xrayLightboxImage');
        lightboxImg.src = preview.src;
        lightbox.classList.add('active');
        document.body.style.overflow = 'hidden';
    }

    function closeXrayLightbox(event) {
        const lightbox = document.getElementById('xrayLightbox');
