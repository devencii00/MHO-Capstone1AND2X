<div class="bg-white border border-slate-200 rounded-[18px] p-5 shadow-[0_2px_10px_rgba(15,23,42,0.04)]">
    ...
    <div class="mb-3 flex flex-col gap-2 md:flex-row md:items-end">
        <div class="flex-1">
            ...
            <input id="admin_medbg_search" type="text" class="w-full rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs text-slate-800 focus:border-green-500 focus:ring-2 focus:ring-green-200 outline-none" placeholder="Patient name or email">
        </div>
        <div class="w-full md:w-56">
            <label for="admin_medbg_category" class="block text-[0.7rem] text-slate-600 mb-1">Category</label>
            <select id="admin_medbg_category" class="w-full rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs text-slate-800 focus:border-green-500 focus:ring-2 focus:ring-green-200 outline-none">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function loadRows() {
            ...
            apiFetch("{{ url('/api/medical-backgrounds') }}?per_page=15", { method: 'GET' })
        ...
        function renderRows() {
            ...
            if (!filtered.length) {
                tableBody.innerHTML = '<tr><td colspan="5" class="py-12 text-center"><div class="flex flex-col items-center justify-center gap-2.5"><div class="w-11 h-11 rounded-full bg-slate-100 flex items-center justify-center text-slate-300"><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 12h-6l-2 3h-4l-2-3H2"/><path d="M5.45 5.11 2 12v6a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-6l-3.45-6.89A2 2 0 0 0 16.76 4H7.24a2 2 0 0 0-1.79 1.11z"/></svg></div><span class="text-sm font-medium text-slate-400">No entries found.</span></div></td></tr>'
                return
            }
