  ...
  return (
    <View style={styles.appointmentCard}>
      <View style={styles.appointmentTopRow}>
        ...
        <View
          style={[styles.pill, { backgroundColor: colors.backgroundColor }]}
        >
      ...
      <View style={styles.detailList}>
        {item.isDependentAppointment ? (
          <View style={styles.detailRow}>
            ...
            >{`Dependent appointment: appointment for "${item.patientName}"`}</Text>
          </View>
        ) : null}
        <View style={styles.detailRow}>
        ...
        <View style={styles.detailRow}>
          ...
          <Text style={styles.detailText}>
            {item.services.length > 0
              ? item.services.join(", ")
              : "Service details not specified yet."}
          </Text>
        </View>
      </View>
    </View>
  ...
  return (
    <View style={styles.emptyState}>
      ...
      <Text style={styles.emptyText}>
        Only appointments with pending or confirmed status appear here.
      </Text>
    </View>
  );
}
    ...
    async function load() {
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}/appointments?upcoming_only=1&per_page=50&order=oldest`,
          {
            headers: {
          ...
          .map((appointment: any) => {
            ...
            const statusRaw =
              typeof appointment?.status === "string"
                ? appointment.status.toLowerCase()
                : "";
            const patientFirst = appointment?.patient?.firstname
              ? String(appointment.patient.firstname)
              : "";
            const patientLast = appointment?.patient?.lastname
              ? String(appointment.patient.lastname)
              : "";
            const patientName =
              [patientFirst, patientLast].filter(Boolean).join(" ").trim() ||
              "Dependent";
            const patientId = Number(
              appointment?.patient_id ?? appointment?.patient?.user_id ?? 0,
            );
            ...
            return {
              ...
              reason:
                ...
                appointment.reason_for_visit.trim().length > 0
                  ? appointment.reason_for_visit.trim()
                  : "Reason for visit not provided.",
              services: services as string[],
              queueNumber:
                appointment?.queue?.queue_number != null
                  ? String(appointment.queue.queue_number)
                  : null,
              patientName,
        ...
        <View style={styles.contentSurface}>
          ...
          >
            {items.length === 0 ? (
              ...
              items.map((item) => (
                <AppointmentDetailsCard key={item.id} item={item} />
              ))
            )}
          </SectionCard>
        </View>
